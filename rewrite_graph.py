...
class ForecastGitGraph extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final double fixedLeftWidth = 80.0;
    ...
    final double totalHeight = 440.0;
    ...
    return Row(
      ...
      children: [
        ...
        SizedBox(
          ...
          height: totalHeight,
          child: CustomPaint(
            painter: FixedLabelsPainter(activeTracks),
          ),
        ),
...
class FixedLabelsPainter extends CustomPainter {
  ...
  FixedLabelsPainter(this.activeTracks);
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // 2. Draw Fixed Row Labels for Balances
    double balancesStartY = _startY + (activeTracks.length * _trackSpacing) + 120;
    double currentTextY = balancesStartY;
    
    if (activeTracks.contains(TrackType.smile)) {
      _drawText(canvas, 'Smile:', size.width - 8, currentTextY, TrackColors.smile, fontSize: 12, bold: true, align: TextAlign.right);
      currentTextY += 18;
    }
    if (activeTracks.contains(TrackType.splurge)) {
      _drawText(canvas, 'Splurge:', size.width - 8, currentTextY, TrackColors.splurge, fontSize: 12, bold: true, align: TextAlign.right);
      currentTextY += 18;
    }
    if (activeTracks.contains(TrackType.fire)) {
      _drawText(canvas, 'Fire:', size.width - 8, currentTextY, TrackColors.fire, fontSize: 12, bold: true, align: TextAlign.right);
      currentTextY += 18;
    }
    if (activeTracks.contains(TrackType.mojo)) {
      _drawText(canvas, 'Mojo:', size.width - 8, currentTextY, TrackColors.mojo, fontSize: 12, bold: true, align: TextAlign.right);
      currentTextY += 18;
    }
    if (activeTracks.contains(TrackType.debt)) {
      _drawText(canvas, 'Debt:', size.width - 8, currentTextY, TrackColors.debt, fontSize: 12, bold: true, align: TextAlign.right);
      currentTextY += 18;
    }
  }
  ...
  void paint(Canvas canvas, Size size) {
    for (int i = 0; i < nodes.length; i++) {
      ...
      if (i == 0) {
        ...
        double textStartY = _startY + (activeTracks.length * _trackSpacing) + 20;
      ...
      // Bucket Balances (Numbers ONLY!)
      double balancesStartY = _startY + (activeTracks.length * _trackSpacing) + 120;
      currentTextY = balancesStartY;
